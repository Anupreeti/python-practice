...
def save_file(contacts):
    ...
    backup_file = f"contacts_{timestamp}.json"
    if os.path.exists("contacts.json"):
        shutil.copy("contacts.json", f"backups/{backup_file}")
...
def add_contact(name, phone, email):
    ...
    save_file(contacts)
    logging.info(f"Contact Added {data}")

def view_contacts():
    contacts = load_file()
    if not contacts:
        logging.error("There is no contact in the file")
    else:
